...
export class RingTimerGraphics {
	...
	private determineWarmUpAlpha(): void {
		...
		const alpha = ((this.warmUpRemaining) / this.config.warmUpFlashTime) % 1;
	...
	private addDropShadow(): void {
		...
		const defs = this.micTimerGroup.append('defs');
	...
	private drawFirstCircle(): void {
		...
		const firstCircle = this.micTimerGroup.selectAll('.firstCircle')
			.data(this.timeData, (d, i) => d.idx);
	...
	private drawSecondCircle(): void {
		...
		const secondCircle = this.micTimerGroup.selectAll('.secondCircle')
			.data(this.timeData, (d, i) => d.idx);
	...
	private drawCounterText(): void {
		...
		const singleDecValueIdx = this.getCounterDecValueIdx();
		const textShift = this.config.calc.counterTextPixels * 0.15;

		const ct = this.micTimerGroup.selectAll('.counterText')
		...
		ct.enter()
			...
			.merge(ct)
			.style('fill', (d, i) => self.getTextColor(self.config.calc.phase))
			.text((d) => self.getCounterText(d));
	...
	private drawTimeUnitsText(): void {
		const singleDecValueIdx = this.getCounterDecValueIdx();
		const shiftTimeUnitsText = this.config.calc.counterTextPixels * 0.25;

		const timeUnitsText = this.micTimerGroup.selectAll('.timeUnitsText')
			.data([this.timeData[singleDecValueIdx]]);
	...
	private circleArcFunction(circleName: string): any {
		const self = this;
		const secondCircleArc = d3.arc()
			.startAngle(this.gaugeCircleX(0))
			.endAngle((d) => self.getEndAngle(d, circleName))
			.innerRadius((d) => self.getInnerRadius(d))
			.outerRadius((d) => self.getOuterRadius(d));
	...
	private getCircleColor(key: string): string {
		const itemColor = this.config.colors[key];
		return this.applyAlpha(key, itemColor);
	}

	private getTextColor(key: string): string {
		const itemColor = this.config.colors.text;
		return this.applyAlpha(key, itemColor);
	}
	...
	private getCounterDecValueIdx(): number {
		let singleDecValueIdx = 0;
		for (const td of this.timeData) {
	...
	private getCounterText(d: any) {
		if (this.config.calc.phase === 'warmup') {
			...
			const retVal = (d.singleDecValue.toFixed(2) < 1.0) ? d.singleDecValue.toFixed(2) : d.singleDecValue.toFixed(1);
			return retVal;
		}
	}
}
